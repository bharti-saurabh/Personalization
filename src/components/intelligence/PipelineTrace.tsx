/**
 * The seven-step pipeline view.
 *
 * This is the engineering read of a single inference: context in, features,
 * inference, probabilities, rules, activation - animated once so a viewer can
 * see the order things happen in. It answers "how does one prediction get made".
 *
 * It deliberately does NOT answer "why does this shopper see this page", which
 * is a question about a whole session rather than one inference. That is what
 * JourneyNarrative is for, and why this view is no longer the panel's front door.
 */
...
export const PipelineTrace: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Run status and re-trace live with the animation, so they sit with it
          rather than in the panel's global header. */}
      <div className="shrink-0 px-4 py-2 bg-white border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2">
          {isAnalyzing ? (
            <span className="bg-amber-100 text-amber-800 text-[10px] px-1.5 py-0.5 rounded border border-amber-300 font-mono font-bold flex items-center gap-1">
              <RotateCw className="h-3 w-3 text-amber-600 animate-spin" />
          ...
          <span className="text-[10px] text-slate-500">Single-inference walkthrough</span>
        </div>
        <button
          onClick={runTraceSequence}
          disabled={isAnalyzing}
          title="Re-run the step animation"
      ...
      <div className="h-1 bg-slate-200 w-full shrink-0 overflow-hidden">
        <div
          className={`h-full transition-all duration-300 ${isAnalyzing ? 'bg-amber-500' : 'bg-emerald-500'}`}
          style={{ width: `${(visibleStep / 7) * 100}%` }}
        />
      </div>

      <div className="flex-1 overflow-y-auto text-xs font-sans bg-slate-50 text-slate-800">
